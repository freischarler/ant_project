...
class Video():
    ...
    def cargar_default(self):
        ...
        try:
            ...
            self.windows_x=int(archivo.readline())
            self.windows_y=int(archivo.readline())
            comprimir=archivo.readline()
            if(comprimir[0]=="y"): self.modo_comprimir=1
            print("Resolution: "+str(self.windows_x)+str(self.windows_y))
            archivo.close()
        except:
        ...
        try:
            ...
            if(txt_i[0]=="y"): self.modo_offscreen=1
                ...
                if(txt_f[0]=="y"): self.modo_fullscreen=1
                    ...
                    self.windows_posx=int(archivoRES.readline().replace('\n', ''))
                    self.windows_posy=int(archivoRES.readline().replace('\n', ''))
                    self.resize=int(archivoRES.readline().replace('\n', '')) 
            archivoRES.close()
        except:
